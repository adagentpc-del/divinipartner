...
- `lib/` - business logic and engines. Key files:
  - Money: `pricingMath.ts` (pure on-top math), `monetization.ts`, `fees.ts`, `platformFees.ts`, `leakage.ts`, `revenueLeakage.ts`, `payoutEngine.ts`, `partnerCommission.ts`, `split-engine.ts`, `stripe-connect.ts` (v1 Express Connect), `processors.ts` (Stripe/PayPal checkout, v1 destination charges), `stripeBilling.ts` (card-based platform subscriptions), `stripeAccounts.ts` (Accounts v2: direct-charge checkout + balance-funded subscriptions, added 2026-08-03 -- see `22_APIS_AND_INTEGRATIONS.md`).
  - Auth/security: `passwordHash.ts`, `session.ts`, `rateLimit.ts`, `securityHeaders.ts`, `csrf.ts` (double-submit CSRF token), `botGuard.ts` (anti-scraper/AI-crawler guard on the public surface), `safe-fetch.ts`, `uploadGuard.ts`, `trust.ts`.
  - Storage: `objectStorage.ts`, `storageCrypto.ts`, `s3sigv4.ts`.
  - Intelligence: `diviniScore.ts`, `playbooks.ts`, `relationshipGraph.ts`, `partnershipMatch.ts`, `commandCenter.ts`, `cooBriefing.ts`, `cooTasks.ts`, `forecasting.ts`, `businessReview.ts` (renamed from `businessHealth.ts` — Divini Business Review, build-order slice 12), `marketplaceIntel.ts`, `pricingIntel.ts`, `revenueIntel.ts`, `eventWarRoom.ts`, `eventMemory.ts`, `recommend.ts`, `nextbestaction.ts`, `promptSafety.ts` (fences untrusted text before it reaches the local LLM).
  - Other: `email.ts`, `notify.ts`, `pdf.ts`, `scheduler.ts`, `extract.ts`, `geo.ts`/`geoip.ts`, `llm.ts` (local-first Ollama client).
  - Twelve deterministic business tools live under `db/` as their own modules (pipeline, scopeBuilder, proposalStudio, followUpDesk, profitMap, priceGuide, compare, changeorders, vendorScorecard, warroom, revenue-intel, business-review) — see `docs/DIVINI_DETERMINISTIC_TOOLS_SPEC.md` for the full build order and each one's shipped report. No `features.tsx` exists on the server; that was a stale doc reference to a frontend-only module (`src/lib/features.tsx`) that was itself removed 2026-08-03 as dead code (its `FeaturesProvider` was never mounted in `App.tsx`).
