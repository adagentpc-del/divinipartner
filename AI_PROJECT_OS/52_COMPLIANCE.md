...
- `AI_PROJECT_OS/53_SOC2_ISO27001_AUDIT.md`: a code-level technical-controls audit mapped to SOC 2 Trust Services Criteria and ISO 27001 Annex A (2026-08-03, updated same day as MFA and the automated-backup mechanism were built). Documents what is actually implemented (access control incl. TOTP MFA enforced for admins, session management, encryption, audit logging, data-subject rights, input/output safety, availability incl. backup + verified restore) and ranks the real open gaps that remain (installing the backup cron schedule, no structured logging/monitoring, no general session revocation, encryption at rest is opt-in).
