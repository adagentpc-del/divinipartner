...
/**
 * AdminManageVendors - create vendor claim profiles and invite owners to claim
 * them. Admin-only. Reads GET /admin/manage/listings?kind=vendor; creates via
...
const STYLES = `
.aml{--emerald:#1E5D4A;--emerald-deep:#123c2e;--emerald-mid:#174838;--gold:#C9A35B;--champagne:#D9CCB0;--ink:#2c2a26;--muted:#6b6459;--line:#e7e1d6;--ivory:#f7f4ee;background:var(--ivory);color:var(--ink);min-height:100vh;font-family:Inter,system-ui,sans-serif}
...
export default function AdminManageVendors() {
  ...
  async function load() {
    setLoadingRows(true);
    try {
      const r = await apiGet<{ listings: Listing[] }>('/admin/manage/listings?kind=vendor');
      setRows(r.listings);
    } catch (e) {
      setMsg({ kind: 'err', text: e instanceof Error ? e.message : 'Failed to load vendors.' });
    } finally {
  ...
  async function create(e: React.FormEvent) {
    ...
    try {
      ...
      setMsg({
        kind: 'ok',
        text: `Vendor profile created.${out.invited ? ' Invite sent to the vendor.' : ' No invite sent.'}`,
      });
      setBusinessName('');
      setCategory('');
      setContactEmail('');
      ...
      setMsg({ kind: 'err', text: err instanceof Error ? err.message : 'Failed to create vendor.' });
    } finally {
      setBusy(null);
    }
  }
  ...
  return (
    <div className="aml">
      ...
      <div className="wrap">
        ...
        <div className="sectitle">Create vendor</div>
        <div className="card">
          ...
          <p className="seghint">
            {setupMode === 'create'
              ? 'You build out the vendor page. No invite is sent by default.'
              : 'The vendor claims their own page. An invite email is sent by default.'}
          </p>
          <form onSubmit={create}>
            <div className="row3">
              ...
              <div>
                ...
                <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Catering, florals, photography..." />
              </div>
              <div>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <textarea rows={4} value={description} onChange={(e) => { setDescription(e.target.value); setAiDraft(false); }} placeholder="Short public description of this vendor. Use Pull from website to auto-draft, then edit." />
            </div>
            ...
            <div style={{ marginTop: 12 }}>
              <button className="btn primary" type="submit" disabled={busy === 'create'}>Create vendor</button>
            </div>
          </form>
        </div>
        ...
        {lastResult && (
          <div className="claimbox">
            ...
            <p className="cmeta">{lastResult.invited ? 'Invite email was sent to the vendor.' : 'No invite email was sent. Share the link above.'}</p>
          </div>
        )}

        <div className="sectitle">Existing vendors</div>
        <div className="card">
          <table>
            <thead>
              <tr><th>Vendor</th><th>Category</th><th>Email</th><th>Location</th><th>Claim status</th><th>Agreement</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {rows.map((r) => (
              ...
              {loadingRows && <tr><td colSpan={7} className="status">Loading vendors...</td></tr>}
              {!loadingRows && !rows.length && <tr><td colSpan={7} className="status">No vendors yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
