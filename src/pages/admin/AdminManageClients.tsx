...
/**
 * AdminManageClients - create client claim profiles and invite owners to claim
 * them. Admin-only. Reads GET /admin/manage/listings?kind=client; creates via
...
export default function AdminManageClients() {
  ...
  async function load() {
    setLoadingRows(true);
    try {
      const r = await apiGet<{ listings: Listing[] }>('/admin/manage/listings?kind=client');
      setRows(r.listings);
    } catch (e) {
      setMsg({ kind: 'err', text: e instanceof Error ? e.message : 'Failed to load clients.' });
    } finally {
  ...
  async function create(e: React.FormEvent) {
    ...
    try {
      ...
      setMsg({
        kind: 'ok',
        text: `Client profile created.${out.invited ? ' Invite sent to the client.' : ' No invite sent.'}`,
      });
      setBusinessName('');
      setCategory('');
      setContactEmail('');
      ...
      setMsg({ kind: 'err', text: err instanceof Error ? err.message : 'Failed to create client.' });
    } finally {
      setBusy(null);
    }
  }
  ...
  return (
    <div className="aml">
      ...
      <div className="wrap">
        ...
        <div className="sectitle">Create client</div>
        <div className="card">
          ...
          <p className="seghint">
            {setupMode === 'create'
              ? 'You build out the client page. No invite is sent by default.'
              : 'The client claims their own page. An invite email is sent by default.'}
          </p>
          <form onSubmit={create}>
            <div className="row3">
              ...
              <div>
                ...
                <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Corporate, private, nonprofit..." />
              </div>
              <div>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <textarea rows={4} value={description} onChange={(e) => { setDescription(e.target.value); setAiDraft(false); }} placeholder="Short public description of this client. Use Pull from website to auto-draft, then edit." />
            </div>
            ...
            <div style={{ marginTop: 12 }}>
              <button className="btn primary" type="submit" disabled={busy === 'create'}>Create client</button>
            </div>
          </form>
        </div>
        ...
        {lastResult && (
          <div className="claimbox">
            ...
            <p className="cmeta">{lastResult.invited ? 'Invite email was sent to the client.' : 'No invite email was sent. Share the link above.'}</p>
          </div>
        )}

        <div className="sectitle">Existing clients</div>
        <div className="card">
          <table>
            <thead>
              <tr><th>Client</th><th>Category</th><th>Email</th><th>Location</th><th>Claim status</th><th>Agreement</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {rows.map((r) => (
              ...
              {loadingRows && <tr><td colSpan={7} className="status">Loading clients...</td></tr>}
              {!loadingRows && !rows.length && <tr><td colSpan={7} className="status">No clients yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
