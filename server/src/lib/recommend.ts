...
// ---------------------------------------------------------------------------
// The implementation lives in the pure, dependency-free eventScope.ts (same
// rationale as pricingMath.ts: unit-testable without a DB/config side effect).
// Re-exported here so existing callers (routes/intelligence.ts) are unaffected.
export {
  buildEventScope,
  type ScopeCategory,
  type ChecklistItem,
