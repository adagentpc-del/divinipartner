/**
 * Event Inventory math (live-ops phase, Part 17-20, 2026-08-09). Pure, no
 * DB -- matches the lib/arrivalStatus.ts / lib/activityVisibility.ts
 * convention.
 *
 ...
 * actionable:
 *   1. Some of an item has arrived (currentTotal > 0) but it is still
 *      short of expected_quantity by more than 10% -- a genuine partial-
 *      delivery shortage, not "the first unit hasn't shown up yet."
 ...
 * Damage/missing-based alerts are added once Part 21-22's count-out
 * workflow exists (those quantities do not exist yet at this point in the
 * build order) -- this function is the one place to extend, not a new
 * alert generator.
 */
export function inventoryAlerts(
  ...
  const shortfall = item.expected_quantity - currentTotal;
  const shortfallRatio = shortfall / item.expected_quantity;
  if (currentTotal > 0 && shortfallRatio > 0.1) {
    alerts.push({
      item_id: item.id,
      severity: shortfallRatio > 0.5 ? "critical" : "warning",
      message: `${item.name}: ${currentTotal} of ${item.expected_quantity} expected have arrived (${Math.round(shortfallRatio * 100)}% short).`,
    });
