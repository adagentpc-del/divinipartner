...
async function gatherMetrics(vendorId: string): Promise<VendorScorecardMetrics> {
  ...
  //
  // status in ('submitted','published') matches the same filter
  // computeTrustForOrg() and listPublishedReviewsForTarget() already use
  // (db/reviews.ts) -- db/schema-phase7.sql documents 'requested'/'draft'/
  // 'hidden' as real lifecycle states alongside those two, so this stays
