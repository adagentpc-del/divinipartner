...
| Confirm whether production actually sets `STORAGE_PROVIDER=s3` (off-host backup storage) and `STORAGE_ENCRYPTION_KEY` (encryption at rest) | Requires reading live production environment variables, not visible from this environment | `compliance/policies/backup-and-restore-runbook.md` | P1 |
| Periodically re-rehearse a real restore (not just once) on a recurring schedule, e.g. quarterly | An untested-since-built control degrades in confidence over time; needs an owner and a calendar reminder, not a one-time AI-run exercise | `compliance/policies/backup-and-restore-runbook.md` (live-verified once, 2026-08-08) | P2 |
| Confirm whether production actually runs a local Ollama instance (or has enabled the opt-in `openai-compat` external-provider mode) | Requires reading live production environment variables and infra state, not visible from this environment | `docs/platform-standard/section-08-ai-security-governance.md` | P2 |
| Provide real Stripe TEST-mode keys (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PUBLISHABLE_KEY`) in a sandboxed environment to exercise real Checkout/webhook/proration/dunning/Connect-payout flows end to end | This environment has zero processor credentials configured (by design, T7); live processor round trips cannot be tested without them | `docs/platform-standard/section-09-payments-stripe-webhooks.md` | P0, before T7 unblocks real money |
| Build and ship a refund-issuance action and a dispute (`charge.dispute.*`) webhook handler before real money goes live | No code path exists today; acceptable only while real money remains off (T7) | `docs/platform-standard/section-09-payments-stripe-webhooks.md`, risk R-33, task T30 | P1, before T7 unblocks |
| Register a Resend delivery-event webhook (`email.bounced`, `email.complained`) pointed at `/api/e/webhook/resend` and set `RESEND_WEBHOOK_SECRET` in production | Requires the Resend dashboard (vendor console access); the code is built and fails closed until this is done | `docs/platform-standard/section-10-email-sms-push-marketing.md` | P1 |
| Run `db/schema-fix-org-fee-rates.sql` (or `db/apply-all.sql`) against staging/production to backfill any `client`/`installer`/`sponsor` org already created with the wrong `platform_fee_rate` | Requires production database access, not available from this environment; idempotent and safe to run regardless of whether any row is currently affected | `docs/platform-standard/section-12-core-product-engines.md`, risk R-41 | P2 (no real money live yet; do before T7 unblocks) |
| Confirm off-host backup storage (`STORAGE_PROVIDER=s3` or equivalent) and off-host secrets recovery are actually configured, then run a real dry-run exercise of `compliance/policies/disaster-recovery-runbook.md` (host provisioning through DNS cutover) | Requires production infra access and a disposable/staging host to safely destroy-and-recover, neither available from this environment | `docs/platform-standard/section-14-observability-incident-response-dr.md`, risk R-45 | P2 |
