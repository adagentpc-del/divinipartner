...
const ThemeBody = z.object({
  ...
  mainLogoStorageKey: z.string().nullable().optional(),
  mainLogoUrl: SafeUrl,
  secondaryLogoStorageKey: z.string().nullable().optional(),
  secondaryLogoUrl: SafeUrl,
  mainLogoDisplayMode: z.enum(["full_header_banner", "contained_logo", "hero_overlay_logo"]).optional(),
  secondaryLogoPlacement: z.enum(["footer", "under_cart", "checkout_sidebar", "footer_and_cart", "hidden"]).optional(),
  headerLogoMaxHeight: z.number().int().min(32).max(400).optional(),
  headerLogoWidthPercent: z.number().int().min(20).max(100).optional(),
  headerAlignment: z.enum(["left", "center", "right"]).optional(),
  headerObjectFit: z.enum(["contain", "cover"]).optional(),
  headerPaddingTop: z.number().int().min(0).max(400).optional(),
  headerPaddingBottom: z.number().int().min(0).max(400).optional(),
  headerBackgroundColor: z.string().nullable().optional(),
  headerGlowEnabled: z.boolean().optional(),
  animationLevel: z.enum(["none", "subtle", "premium"]).optional(),
