...
import { db, suppliersTable, productCatalogTable, withMmColumns, withWeightColumns } from "@workspace/db";
import { z } from "zod";
...
const ResourceParam = z.enum(["suppliers", "products", "specs"]);
...
router.post("/imports/commit", async (req: Request, res: Response): Promise<void> => {
  ...
  const { resource, mode, rows } = parsed.data;
  try {
    const result = resource === "suppliers"
      ? await commitSuppliers(rows, mode)
      : resource === "products"
      ? await commitProducts(rows, mode)
      : await commitSpecs(rows, mode);
    res.json(result);
  } catch (e: any) {
