...
- **Security**: Includes file upload validation, SSRF mitigation, detailed error handling, CSV sanitization, production security hardening, rate limiting, and secure secret management.
- **Cost & AI Architecture**: AI is used for specific narrow tasks (request summaries, pitch-deck normalization, package PDF extraction, billing PDF signal extraction) via `gpt-4o-mini`, with structured JSON outputs, capped `max_tokens`, and content-hash caching to minimize spend. Deterministic logic is intentionally excluded from AI processing.
