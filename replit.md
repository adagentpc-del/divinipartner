...
- **Sales Intake & Opportunity Routing**: A standalone sales module layered on the existing stack (Postgres + Drizzle + Clerk + Resend + Object Storage — no Supabase). Roles: Super Admin (Alyssa) sees all; Sales Reps (Drew, Retta) see only their own accounts/opportunities, scoped via `requireSalesUser`/`requireSuperAdmin` middleware and `scopedOppWhere(user)`. Clerk email maps to a `sales_reps` row; `ADMIN_ALLOWED_EMAILS` bootstrap as super_admin. Public intake forms (`/intake/{alyssa,drew,retta,general}`, General Project + Pole Banner) submit to `POST /public/intake/submit`, which fuzzy-matches company to an account owner (else link source, else Super Admin queue), creates a submission + an Opportunity, and fires a Resend routing email. Opportunity pipeline has stages, won/lost + lost reasons, notes, and uploads. A Templates & Specs library (Super Admin upload/publish/archive) surfaces client-facing downloads on public intake pages via `GET /public/intake/templates` (safe projection, strips uploader/internal flags). Client-facing template files are uploaded to the PUBLIC object bucket (`/api/storage/public-objects/...`) so anonymous visitors can download them; the server rejects private `/objects/...` paths when `clientFacing=true` (`isPublicFileUrl`). Dashboards (Super Admin org-wide + rep-scoped) and a formula-injection-guarded CSV export round out the module.
- **Venue Asset Survey Integration**: Per-partner connection to A3's external Venue Asset Survey app via HMAC-signed webhook (`/public/integrations/asset-survey/:slug`, header `X-Survey-Signature`, sha256 of raw body) and admin pull (`/admin/integrations/asset-survey/pull/:partnerId`). Assets are stored with public/internal field separation — public projection (`toPublicSurveyAsset`) strips internalNotes, installNotes, productionNotes, internalPhotos, NetSuite asset/venue numbers, surveyor, and the raw payload. Admin Survey Imports inbox approves assets, sets material mode (none / fixed / choose_at_order), and curates an approved-materials shortlist from the master Approved Materials list. Approved assets appear as a "Brand our space" tile gallery on the public portal with optional material dropdown; selections push a cart line carrying `surveyAssetId` + `selectedMaterial`. A3 internal intake (`OrderInternalIntakePanel` + intake email) renders an extra Venue Survey block per matching line showing measurements, selected material, install/production/internal notes, and NetSuite asset/venue numbers.
